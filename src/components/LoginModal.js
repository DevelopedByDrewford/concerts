...
// WKWebView (Chrome/Brave/Firefox/Edge on iOS) isolates IndexedDB per-tab and
// blocks window.opener postMessage — both signInWithPopup and signInWithRedirect
// fail silently. This is an Apple platform restriction with no client-side fix.
// Safari uses its own engine and handles Google OAuth correctly.
function isIosNonSafari() {
  const ua = navigator.userAgent;
  if (!/iPhone|iPad|iPod/.test(ua)) return false;
  return !/Version\//.test(ua) || /CriOS|FxiOS|EdgiOS|OPiOS|GSA/.test(ua);
}
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-sheet" onClick={e => e.stopPropagation()}>
        ...
        {iosNonSafari ? (
          <div className="modal-safari-prompt">
            <p className="modal-safari-prompt__title">
              Sign-in requires Safari on iPhone
            </p>
            <p className="modal-safari-prompt__body">
              Google sign-in doesn't work in {iosBrowserName()} on iPhone. Copy the link and open it in Safari to continue.
            </p>
